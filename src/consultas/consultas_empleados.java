...
public class consultas_empleados extends conexion {
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	public boolean guardar_empleado(empleados empleado, Date fecha_nacimiento, Date fecha_inicio, Date fecha_renuncia) {
    ...
    public empleados obtenerEmpleadoPorId(int idEmpleado) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Error al consultar los datos del empleados", "Error", JOptionPane.ERROR_MESSAGE);
        }
        ...
        try (Connection conn = new conexion().conectar();
            ...
            if (rs.next()) {
                if (rs.getInt("id_empleado") == id_empleado) {
                    campoDuplicado = "Id de empleados";
                } else if (rs.getString("identidad_empleado").equals(identidad_empleado)) {
                    campoDuplicado = "Identidad de empleados";
                }
            }
    ...
    public String empleadoExisteParaActualizar(int id, int nuevoIdEmpleado, String identidad_empleado) {
        ...
        try {
            ...
            ps.setInt(3, id); // Se excluye el empleados que se está editando
    ...
    public boolean actualizar_empleado(empleados emp, int idOriginal, int nuevoIdEmpleado, Date fechaNacimiento, Date fechaInicio, Date fechaRenuncia) {
        Connection con = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            System.out.println("Error al actualizar empleados: " + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
            try {
    ...
    public boolean existeIdEmpleado(int idEmpleado, int idOriginal) {
        ...
        try (Connection conn = conectar(); 
            ...
            stmt.setInt(2, idOriginal); // Excluir el registro del empleados actual
            ResultSet rs = stmt.executeQuery();
            
            if (rs.next()) {
                return rs.getInt(1) > 0; // Si el resultado es mayor a 0, el ID existe en otro empleados
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean existeIdentidadEmpleado(String identidadEmpleado, int idOriginal) {
        ...
        try (Connection conn = conectar(); 
            ...
            if (rs.next()) {
                return rs.getInt(1) > 0; // Si el resultado es mayor a 0, la identidad existe en otro empleados
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public String obtenerIdentidadEmpleado(int idOriginal) {
        ...
        try (Connection conn = conectar(); 
            ...
            stmt.setInt(1, idOriginal); // El ID original del empleados
            ResultSet rs = stmt.executeQuery();
            
            if (rs.next()) {
                identidadEmpleado = rs.getString("identidad_empleado"); // Obtener la identidad del empleados
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<empleados> obtenerNombresEmpleados() {
    ...
    public empleados obtenerDatosEmpleadoPorId(int idEmpleado) {
        empleados emp = null;
        conexion con = new conexion();
        Connection conexion = null;
